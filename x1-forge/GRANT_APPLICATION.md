...
**Technical Roadmap**
- M1 (Weeks 1–2): `create-x1-app` CLI + default template — Hardhat project preconfigured with Maculatus testnet (chain ID 10778), sample `Greeter` contract, deploy + test scripts, wallet-connect frontend demo. [Status: built and verified — CLI scaffolds, `npm test` and `hardhat compile` pass.]
...
- **Milestone 1**
  ...
  - Deliverables & Success Metrics/KPIs: CLI published to npm; running `npx create-x1-app <name>` produces a project where `npm test` and `npm run deploy:testnet` succeed against the live testnet; public GitHub repo.
...
Milestone 1 in progress — CLI and default template built and locally verified (compiles, tests pass, deploy script ready against Maculatus testnet RPC).
...
- The CLI and M1 template already exist in this repo (`x1-forge/`) and have been verified: `create-x1-app` scaffolds correctly, `npm test` passes, `hardhat compile` succeeds against Solidity 0.8.24. Deploying to the live Maculatus testnet still needs a funded wallet (via the Discord faucet) to fully confirm end-to-end.
